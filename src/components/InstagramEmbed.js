...
export default function InstagramEmbed({ postId, className = "" }) {
  ...
  useEffect(() => {
    ...
    if (!postId) {
      const fetchPostId = async () => {
        try {
          const response = await fetch('/api/getPostID');
          const data = await response.json();
          if (data.latestPostID) {
            setDynamicPostId(data.latestPostID);
          }
        } catch (error) {
  ...
  if (loading) {
    return (
      <div className={`flex items-center justify-center p-8 bg-gray-100 rounded-lg ${className}`}>
        <div className="text-gray-500">Loading Instagram post...</div>
      </div>
    );
  }
  ...
  if (!dynamicPostId) {
    return (
      <div className={`flex items-center justify-center p-8 bg-gray-100 rounded-lg ${className}`}>
        <div className="text-gray-500">Unable to load Instagram post</div>
      </div>
    );
  }

  const instagramUrl = `https://www.instagram.com/p/${dynamicPostId}/?utm_source=ig_embed&utm_campaign=loading`;
      ...
      <div style={{ padding: '16px' }}>
        ...
        <p style={{ color: '#c9c8cd', fontFamily: 'Arial,sans-serif', fontSize: '14px', lineHeight: '17px', marginBottom: 0, marginTop: '8px', overflow: 'hidden', padding: '8px 0 7px', textAlign: 'center', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          <a 
            ...
            rel="noopener noreferrer"
          >
            A post shared by Ismael Grimaldo (@ismaelvega.dev)
          </a>
        </p>
      </div>
    </blockquote>
